...
You can verify the policy was applied by running ``kubectl exec -n $CILIUM_NAMESPACE cilium-xxxx -- ciliumctl policy get``
for the Cilium agent pod. Verify that the host is selected by the policy using
``ciliumctl endpoint list`` and look for the endpoint with ``reserved:host`` as the
...
``ciliumctl status list`` output. Use ``ciliumctl monitor`` with ``--related-to`` and
