...
Then start running ciliumctl monitor in "L7 mode" to monitor for HTTP requests being reported by Cilium:

.. code-block:: shell-session

    $ kubectl exec -it -n kube-system cilium-d5x8v -- ciliumctl monitor -t l7
...
Looking back at the ciliumctl monitor window, you will see each individual HTTP request and response.  For example::
