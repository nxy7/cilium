...
.. code-block:: shell-session
    ...
    $ HOST_EP_ID=$(kubectl -n $CILIUM_NAMESPACE exec $CILIUM_POD_NAME -- ciliumctl endpoint list -o jsonpath='{[?(@.status.identity.id==1)].id}')
    $ kubectl -n $CILIUM_NAMESPACE exec $CILIUM_POD_NAME -- ciliumctl endpoint config $HOST_EP_ID PolicyAuditMode=Enabled
    Endpoint 3353 configuration updated successfully
    $ kubectl -n $CILIUM_NAMESPACE exec $CILIUM_POD_NAME -- ciliumctl endpoint config $HOST_EP_ID | grep PolicyAuditMode
...
the output of command ``ciliumctl endpoint list``. You can therefore inspect the
status of the policy using that command.

.. code-block:: shell-session

    $ kubectl -n $CILIUM_NAMESPACE exec $CILIUM_POD_NAME -- ciliumctl endpoint list
...
``ciliumctl monitor`` as ``action audit``. The audit mode thus allows you to
adjust the host policy to your environment, to avoid unexpected connection
breakages.

.. code-block:: shell-session

    $ kubectl -n $CILIUM_NAMESPACE exec $CILIUM_POD_NAME -- ciliumctl monitor -t policy-verdict --related-to $HOST_EP_ID
...
.. code-block:: shell-session
    ...
    $ kubectl -n $CILIUM_NAMESPACE exec $CILIUM_POD_NAME -- ciliumctl endpoint config $HOST_EP_ID PolicyAuditMode=Disabled
    Endpoint 3353 configuration updated successfully
