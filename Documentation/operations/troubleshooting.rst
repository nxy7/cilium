...
automatically report the failure state via ``ciliumctl status`` which is regularly
...
the agent on that node can be retrieved by running ``ciliumctl status`` in the
context of that pod:

.. code-block:: shell-session

   $ kubectl -n kube-system exec cilium-2hq5z -- ciliumctl status
...
Alternatively, the ``k8s-cilium-exec.sh`` script can be used to run ``ciliumctl
...
... and run ``ciliumctl status`` on all nodes:

.. code-block:: shell-session

   $ ./k8s-cilium-exec.sh ciliumctl status
...
``ciliumctl status --verbose`` command. Verbose output includes detailed IPAM state
...
following output in ``ciliumctl status``:
...
``ciliumctl monitor`` allows you to quickly inspect and see if and where packet
drops happen. Following is an example output (use ``kubectl exec`` as in
previous examples if running with Kubernetes):

.. code-block:: shell-session

   $ kubectl -n kube-system exec -ti cilium-2hq5z -- ciliumctl monitor --type drop
...
If connectivity fails and ``ciliumctl monitor --type drop`` shows ``xx drop (CT:
...
``ciliumctl monitor -v`` output. To enable them, add ``"datapath"`` to
...
.. code-block:: shell-session
    ...
    kubectl -n kube-system exec -ti cilium-q8wvt -- ciliumctl endpoint list
...
.. code-block:: shell-session
    ...
    kubectl -n kube-system exec -ti cilium-q8wvt -- ciliumctl endpoint get 59084
...
The etcd status is reported when running ``ciliumctl status``. The following line
 ...
 * If no etcd endpoints can be reached, Cilium will report failure in ``ciliumctl
...
#. Run ``ciliumctl monitor`` on the node of the source and destination endpoint.
   Look for packet drops.

   When running in :ref:`arch_overlay` mode:

#. Run ``ciliumctl bpf tunnel list`` and verify that each Cilium node is aware of
...
#. If nodes are being populated correctly, run ``tcpdump -n -i cilium_vxlan`` on
   ...
   * verify that the node IP listed in ``ciliumctl bpf tunnel list`` can reach each
     other.
...
* ``uptime``
* ``ciliumctl bpf * list``
* ``ciliumctl endpoint get for each endpoint``
...
If you are not running Kubernetes, you can use the ``ciliumctl debuginfo`` command
...
``ciliumctl debuginfo`` can print useful output from the Cilium API. The output
...
.. code-block:: shell-session
   ...
   ciliumctl debuginfo -f debuginfo.md
