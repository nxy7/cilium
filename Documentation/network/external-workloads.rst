...
``cilium`` and copies the ``ciliumctl`` node CLI to your host. This needs
``sudo`` permissions, so you may be asked for a password. Note that
this ``ciliumctl`` command is not the same as the ``cilium`` CLI used to
...
.. code-block:: shell-session
    ...
    ping $(sudo ciliumctl service list get -o jsonpath='{[?(@.spec.flags.name=="clustermesh-apiserver")].spec.backend-addresses[0].ip}')
