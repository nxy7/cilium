...
selected by the policy. Make sure that ``ciliumctl service list`` is run
...
.. code-block:: shell-session
    ...
    $ kubectl exec -it -n kube-system cilium-5ngzd -- ciliumctl service list
...
.. code-block:: shell-session
    ...
    $ kubectl exec -it -n kube-system ds/cilium -- ciliumctl service list
    ID   Frontend               Service Type   Backend
    [...]
...
selected by the policy. Make sure to run ``ciliumctl service list`` in Cilium pod
