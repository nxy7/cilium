...
while ! docker exec -t lb-node docker exec -t cilium-lb ciliumctl status; do sleep 1; done
...
docker exec -t lb-node docker exec -t cilium-lb \
    ciliumctl service update --id 1 --frontend "${LB_VIP}:80" --backends "${WORKER_IP}:80" --k8s-node-port
...
docker exec -t lb-node docker exec -t cilium-lb \
    ciliumctl service update --id 1 --frontend "${LB_VIP}:80" --backends "${WORKER_IP}:80" --backend-weights "0" --k8s-node-port
