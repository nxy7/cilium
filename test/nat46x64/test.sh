...
function cilium_install {
    ...
    docker exec -t lb-node \
        docker run --name cilium-lb -td \
            ...
            "$@"
    while ! ${CILIUM_EXEC} ciliumctl status; do sleep 3; done
    sleep 1
}
...
${CILIUM_EXEC} \
    ciliumctl service update --id 1 --frontend "${LB_VIP}:80" --backends "[${WORKER_IP6}]:80" --k8s-node-port

SVC_BEFORE=$(${CILIUM_EXEC} ciliumctl service list)

${CILIUM_EXEC} ciliumctl bpf lb list

MAG_V4=$(${CILIUM_EXEC} ciliumctl bpf lb maglev list -o=jsonpath='{.\[1\]/v4}' | tr -d '\r')
MAG_V6=$(${CILIUM_EXEC} ciliumctl bpf lb maglev list -o=jsonpath='{.\[1\]/v6}' | tr -d '\r')
if [ ! -z "$MAG_V4" -o -z "$MAG_V6" ]; then
	echo "Invalid content of Maglev table!"
	${CILIUM_EXEC} ciliumctl bpf lb maglev list
	exit 1
fi
...
SVC_AFTER=$(${CILIUM_EXEC} ciliumctl service list)
...
${CILIUM_EXEC} \
    ciliumctl service update --id 2 --frontend "[${LB_ALT}]:80" --backends "[${WORKER_IP6}]:80" --k8s-node-port

${CILIUM_EXEC} ciliumctl service list
${CILIUM_EXEC} ciliumctl bpf lb list
...
${CILIUM_EXEC} ciliumctl service delete 1
${CILIUM_EXEC} ciliumctl service delete 2
...
${CILIUM_EXEC} \
    ciliumctl service update --id 1 --frontend "[${LB_VIP}]:80" --backends "${WORKER_IP4}:80" --k8s-node-port
...
${CILIUM_EXEC} \
    ciliumctl service update --id 2 --frontend "${LB_ALT}:80" --backends "${WORKER_IP4}:80" --k8s-node-port
...
${CILIUM_EXEC} \
    ciliumctl recorder update --id 1 --caplen 100 \
...
${CILIUM_EXEC} \
    ciliumctl recorder update --id 2 --caplen 100 \
...
${CILIUM_EXEC} ciliumctl recorder list
${CILIUM_EXEC} ciliumctl bpf recorder list
${CILIUM_EXEC} ciliumctl recorder delete 1
${CILIUM_EXEC} ciliumctl recorder delete 2
${CILIUM_EXEC} ciliumctl recorder list
