...
func (kub *Kubectl) GetCiliumHostEndpointID(ciliumPod string) (int64, error) {
	cmd := fmt.Sprintf("ciliumctl endpoint list -o jsonpath='{[?(@.status.identity.id==%d)].id}'",
...
func (kub *Kubectl) GetCiliumHostEndpointState(ciliumPod string) (string, error) {
	cmd := fmt.Sprintf("ciliumctl endpoint list -o jsonpath='{[?(@.status.identity.id==%d)].status.state}'",
		ReservedIdentityHost)
	res := kub.CiliumExecContext(context.Background(), ciliumPod, cmd)
	if !res.WasSuccessful() {
...
func (kub *Kubectl) CountMissedTailCalls() (int, error) {
	...
	for _, ciliumPod := range ciliumPods {
		cmd := "ciliumctl metrics list -o json | jq '.[] | select( .name == \"cilium_drop_count_total\" and .labels.reason == \"Missed tail call\" ).value'"
		res := kub.CiliumExecContext(context.Background(), ciliumPod, cmd)
		if !res.WasSuccessful() {
...
// MonitorStart runs ciliumctl monitor in the background and returns the command
...
func (kub *Kubectl) MonitorStart(pod string) (res *CmdRes, cancel func()) {
	cmd := fmt.Sprintf("%s exec -n %s %s -- ciliumctl monitor -vv", KubectlCmd, CiliumNamespace, pod)
	ctx, cancel := context.WithCancel(context.Background())

	return kub.ExecInBackground(ctx, cmd, ExecOptions{SkipLog: true}), cancel
}

// MonitorEndpointStart runs ciliumctl monitor only on a specified endpoint. This
// function is the same as MonitorStart.
func (kub *Kubectl) MonitorEndpointStart(pod string, epID int64) (res *CmdRes, cancel func()) {
	cmd := fmt.Sprintf("%s exec -n %s %s -- ciliumctl monitor -vv --related-to %d",
...
func (kub *Kubectl) validateServicePlumbingInCiliumPod(fullName, ciliumPod string, serviceObj *v1.Service, endpointsObj v1.Endpoints) error {
	...
	cmd := "ciliumctl service list -o json | " + jq
	res := kub.CiliumExecContext(context.Background(), ciliumPod, cmd)
	if !res.WasSuccessful() {
	...
	cmd = "ciliumctl bpf lb list -o json"
	res = kub.CiliumExecContext(context.Background(), ciliumPod, cmd)
	if !res.WasSuccessful() {
		return fmt.Errorf("unable to validate cilium service by running '%s': %s", cmd, res.OutputPrettyPrint())
	}
	...
	if err != nil {
		return fmt.Errorf("unable to unmarshal ciliumctl bpf lb list output: %s", err)
	}
	...
	for _, port := range serviceObj.Spec.Ports {
		...
		if _, ok := lbMap[net.JoinHostPort(serviceObj.Spec.ClusterIP, fmt.Sprintf("%d", port.Port))]; !ok {
			return fmt.Errorf("port %d of service %s (%s) not found in ciliumctl bpf lb list of pod %s",
				port.Port, fullName, serviceObj.Spec.ClusterIP, ciliumPod)
		}
	}
...
// - The service and endpoints are found in `ciliumctl service list`
// - The service and endpoints are found in `ciliumctl bpf lb list`
...
// CiliumEndpointsList returns the result of `ciliumctl endpoint list` from the
// specified pod.
func (kub *Kubectl) CiliumEndpointsList(ctx context.Context, pod string) *CmdRes {
	return kub.CiliumExecContext(ctx, pod, "ciliumctl endpoint list -o json")
}
...
func (kub *Kubectl) CiliumEndpointIPv6(pod string, endpoint string) map[string]string {
	...
	return kub.CiliumExecContext(ctx, pod, fmt.Sprintf(
		"ciliumctl endpoint get %s -o jsonpath='%s'", endpoint, filter)).KVOutput()
}
...
func (kub *Kubectl) LoadedPolicyInFirstAgent() (string, error) {
	...
	for _, pod := range pods {
		ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
		defer cancel()
		res := kub.CiliumExecContext(ctx, pod, "ciliumctl policy get")
		if !res.WasSuccessful() {
...
func (kub *Kubectl) WaitPolicyDeleted(pod string, policyName string) error {
	body := func() bool {
		ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
		defer cancel()
		res := kub.CiliumExecContext(ctx, pod, fmt.Sprintf("ciliumctl policy get %s", policyName))

		// `ciliumctl policy get <policy name>` fails if the policy is not loaded,
...
func (kub *Kubectl) CiliumIsPolicyLoaded(pod string, policyCmd string) bool {
	ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
	defer cancel()
	res := kub.CiliumExecContext(ctx, pod, fmt.Sprintf("ciliumctl policy get %s", policyCmd))
	return res.WasSuccessful()
}
...
func (kub *Kubectl) CiliumPolicyRevision(pod string) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
	defer cancel()
	res := kub.CiliumExecContext(ctx, pod, "ciliumctl policy get -o json")
	if !res.WasSuccessful() {
...
func (kub *Kubectl) waitNextPolicyRevisions(podRevisions map[string]int, timeout time.Duration) error {
	body := func() bool {
		for ciliumPod, revision := range podRevisions {
			...
			res := kub.CiliumExecContext(ctx, ciliumPod, fmt.Sprintf("ciliumctl policy wait %d --max-wait-time %d", desiredRevision, int(ShortCommandTimeout.Seconds())))
...
func (kub *Kubectl) CiliumCheckReport(ctx context.Context) {
	...
	for _, pod := range pods {
		var prefix = ""
		status := kub.CiliumExecContext(ctx, pod, "ciliumctl status --all-controllers -o json")
...
func (kub *Kubectl) ciliumStatusPreFlightCheck() error {
	...
	for _, pod := range ciliumPods {
		status := kub.CiliumExecContext(context.TODO(), pod, "ciliumctl status --all-health --all-nodes")
		if !status.WasSuccessful() {
...
func (kub *Kubectl) ciliumControllersPreFlightCheck() error {
	...
	for _, pod := range ciliumPods {
		status := kub.CiliumExecContext(context.TODO(), pod, fmt.Sprintf(
			"ciliumctl status --all-controllers -o jsonpath='%s'", controllersFilter))
		if !status.WasSuccessful() {
		...
		for controller, status := range status.KVOutput() {
			if status != "0" {
				failmsg := kub.CiliumExecContext(context.TODO(), pod, "ciliumctl status --all-controllers")
...
func (kub *Kubectl) fillServiceCache() error {
	...
	ciliumSvcCmd := "ciliumctl service list -o json"
	ciliumBpfLbCmd := "ciliumctl bpf lb list -o json"
...
func (kub *Kubectl) CiliumServiceAdd(pod string, id int64, frontend string, backends []string, svcType, trafficPolicy string) error {
	...
	ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
	defer cancel()
	return kub.CiliumExecContext(ctx, pod, fmt.Sprintf("ciliumctl service update --id %d --frontend %q --backends %q %s",
		id, frontend, backendsStr, optsStr)).GetErr("ciliumctl service update")
}
...
func (kub *Kubectl) CiliumServiceDel(pod string, id int64) error {
	ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
	defer cancel()
	return kub.CiliumExecContext(ctx, pod, fmt.Sprintf("ciliumctl service delete %d", id)).GetErr("ciliumctl service delete")
}
...
// WaitForIPCacheEntry waits until the given ipAddr appears in "ciliumctl bpf ipcache list"
// on the given node.
func (kub *Kubectl) WaitForIPCacheEntry(node, ipAddr string) error {
	...
	body := func() bool {
		ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
		defer cancel()
		cmd := fmt.Sprintf(`ciliumctl bpf ipcache list | grep -q %s`, ipAddr)
...
func (kub *Kubectl) WaitForEgressPolicyEntries(node string, expectedCount int) error {
	...
	body := func() bool {
		ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
		defer cancel()
		cmd := "ciliumctl bpf egress list | tail -n +2 | wc -l"
...
func (kub *Kubectl) GetDNSProxyPort(ciliumPod string) int {
	...
	// # ciliumctl status -o jsonpath='{.proxy.redirects[?(@.proxy=="cilium-dns-egress")].proxy-port}'
	//
...
// appears in "ciliumctl bpf lb list --frontends" on the given node.
func (kub *Kubectl) WaitForServiceFrontend(nodeName, ipAddr string) error {
	...
	body := func() bool {
		ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
		defer cancel()
		cmd := fmt.Sprintf(`ciliumctl bpf lb list --frontends | grep -q %s`, ipAddr)
		return kub.CiliumExecContext(ctx, ciliumPod, cmd).WasSuccessful()
	}
...
// appears in "ciliumctl bpf lb list --backends" on the given node.
func (kub *Kubectl) WaitForServiceBackend(node, ipAddr string) error {
	...
	body := func() bool {
		ctx, cancel := context.WithTimeout(context.Background(), ShortCommandTimeout)
		defer cancel()
		cmd := fmt.Sprintf(`ciliumctl bpf lb list --backends | grep -q %s`, ipAddr)
		return kub.CiliumExecContext(ctx, ciliumPod, cmd).WasSuccessful()
	}
