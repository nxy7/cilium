...
var _ = Describe("RuntimeAgentFQDNPolicies", func() {
	...
	expectFQDNSareApplied := func(domain string, minNumIDs int) {
		...
		body := func() bool {
			res := vm.Exec(fmt.Sprintf(`ciliumctl policy selectors -o json | %s`, jqfilter))
...
[
  {
	...
	It("Validate dns-proxy monitor information", func() {
		...
		monitorCMD := vm.ExecInBackground(ctx, "ciliumctl monitor --type=l7")
		defer cancel()
...
[
  {
	...
	Context("With verbose policy logs", func() {
		...
		BeforeEach(func() {
			By("Clearing fqdn cache: %s", vm.Exec("ciliumctl fqdn cache clean -f").CombineOutput().String())
		})
...
[
	...
	Context("toFQDNs populates toCIDRSet (data from proxy)", func() {
		...
		It("Policy addition after DNS lookup", func() {
			ctx, cancel := context.WithCancel(context.Background())
			monitorCMD := vm.ExecInBackground(ctx, "ciliumctl monitor")
			defer cancel()
