...
var _ = Describe("RuntimeAgentKVStoreTest", func() {
	...
	AfterFailed(func() {
		vm.ReportFailed("ciliumctl status")
	})
...
var kvstoreChaosTests = func() {
	...
	It("Delete event on KVStore with CIDR identities", func() {
		...
		CIDRIdentities := vm.Exec(fmt.Sprintf(`ciliumctl identity list -o json| %s`, jqFilter))
		...
		newCIDRIdentities := vm.Exec(fmt.Sprintf(`ciliumctl identity list -o json| %s`, jqFilter))
