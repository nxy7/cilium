	...
	Context("With Sample Containers", func() {
		...
		It("Cilium monitor event types", func() {
			...
			for k, v := range eventTypes {
				...
				defer cancel()
				res := vm.ExecInBackground(ctx, fmt.Sprintf("ciliumctl monitor --type %s -vv", k))
		...
		It("ciliumctl monitor check --from", func() {
			monitorConfig()
			...
			res := vm.ExecInBackground(ctx, fmt.Sprintf(
				"ciliumctl monitor --type debug --from %s -vv", endpoints[helpers.App1]))
			vm.ContainerExec(helpers.App1, helpers.Ping(helpers.Httpd1))
		...
		It("ciliumctl monitor check --to", func() {
			monitorConfig()
			...
			defer cancel()
			res := vm.ExecInBackground(ctx, fmt.Sprintf(
				"ciliumctl monitor -vv --to %s", endpoints[helpers.Httpd1]))
		...
		It("ciliumctl monitor check --related-to", func() {
			monitorConfig()
			...
			defer cancel()
			res := vm.ExecInBackground(ctx, fmt.Sprintf(
				"ciliumctl monitor -vv --related-to %s", endpoints[helpers.Httpd1]))
		...
		It("delivers the same information to multiple monitors", func() {
			...
			for i := 1; i <= 3; i++ {
				monitorRes = append(monitorRes, vm.ExecInBackground(ctx, "ciliumctl monitor"))
			}
		...
		It("checks container ids match monitor output", func() {
			...
			ctx, cancel := context.WithCancel(context.Background())
			res := vm.ExecInBackground(ctx, "ciliumctl monitor -vv")
