...
var _ = SkipDescribeIf(func() bool { return helpers.RunsOn54Kernel() && helpers.DoesNotRunOnAKS() }, "K8sDatapathLRPTests", func() {
	...
	AfterFailed(func() {
		kubectl.CiliumReport("ciliumctl lrp list", "ciliumctl service list")
	})
	...
	SkipContextIf(func() bool { return !helpers.RunsOn419OrLaterKernel() && helpers.DoesNotRunOnAKS() }, "Checks local redirect policy", func() {
		...
		It("LRP connectivity", func() {
			...
			for _, pod := range ciliumPods {
				service := kubectl.CiliumExecMustSucceed(context.TODO(), pod, fmt.Sprintf("ciliumctl service list | grep \" %s:\"", svcIP), "Cannot retrieve services on cilium pod")
				service.ExpectContains("LocalRedirect", "LocalRedirect is not present in the cilium service list for [%s]", svcIP)
				service2 := kubectl.CiliumExecMustSucceed(context.TODO(), pod, fmt.Sprintf("ciliumctl service list | grep \" %s:\"", lrpAddrIP), "Cannot retrieve services on cilium pod")
